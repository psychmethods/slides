...
[Download the Excel file here](https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/IrisData.xlsx)

**Answer Key**: [IrisData_Answers.xlsx](https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/IrisData_Answers.xlsx)
]
.pull-right-narrow[
.center[
...
share_link <- "https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/IrisData.xlsx"
...
.footnote[or here https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/IrisData.xlsx]
...
# R Verification
- If you want to verify the calculations in R, you can use the following code:
```{r echo=TRUE}
...
```{r echo=TRUE}
t_statistic <- (mean(iris$Sepal.Length) - 5.5) / (sd(iris$Sepal.Length) / sqrt(nrow(iris)))
...
# R Verification
```{r echo=TRUE}
p_value <- 2 * pt(-abs(t_statistic), df = nrow(iris) - 1)
p_value
...
[Download the Excel file here](https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/Iris_TwoSample_TTest.xlsx)

**Answer Key**: [Iris_TwoSample_TTest_Answers.xlsx](https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/Iris_TwoSample_TTest_Answers.xlsx)

```{r, echo=FALSE, out.width="50%", warning = FALSE}
library(qrcode)

share_link <- "https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/Iris_TwoSample_TTest.xlsx"
code <- qr_code(share_link)
...
mean(data_virginica$Sepal.Length)

data_versicolor <- iris %>%
  filter(Species == "versicolor")

mean(data_versicolor$Sepal.Length)
```
...
```{r echo=TRUE}
sd(data_virginica$Sepal.Length)
sd(data_versicolor$Sepal.Length)
```
...
1. **Select a cell** (e.g., B156) to calculate the sample size of **Virginica**:
   - In the **Formula Bar**, type `=COUNT(A2:A52)`.
...
image_path <- "../img/excel_lab/excel screenshot_zoom.png"
img <- image_read(image_path ) %>%
  image_annotate("=COUNT(A2:A52)", size = 30, color = "darkgreen", location = "+550+475")
...
t_statistic_two_sample <- (mean(data_virginica$Sepal.Length) - mean(data_versicolor$Sepal.Length)) / 
  sqrt((sd(data_virginica$Sepal.Length)^2 / nrow(data_virginica)) + 
         (sd(data_versicolor$Sepal.Length)^2 / nrow(data_versicolor)))


t_statistic_two_sample
```
...
```{r echo=TRUE}
df_two_sample <- (((sd(data_virginica$Sepal.Length)^2 / nrow(data_virginica)) + 
                     (sd(data_versicolor$Sepal.Length)^2 / nrow(data_versicolor)))^2) / 
  (((sd(data_virginica$Sepal.Length)^2 / nrow(data_virginica))^2 / (nrow(data_virginica) - 1)) + 
     ((sd(data_versicolor$Sepal.Length)^2 / nrow(data_versicolor))^2 / (nrow(data_versicolor) - 1)))
...
```{r echo=TRUE}
p_value_two_sample <- 2 * pt(-abs(t_statistic_two_sample), df = df_two_sample)
...
t.test(Sepal.Length ~ Species, data = data_vs)
```
...
2. **Download the file here**: [Download Link](https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/Iris_ANOVA.xlsx).

3. **Answer Key**: [Iris_ANOVA_Answers.xlsx](https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/Iris_ANOVA_Answers.xlsx)

.pull-right[
```{r, echo=FALSE, out.width="50%", warning = FALSE}
library(qrcode)

share_link <- "https://github.com/psychmethods/slides/raw/refs/heads/main/t14_anovalab/Iris_ANOVA.xlsx"
code <- qr_code(share_link)
