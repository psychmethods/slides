...
- Distinguish between robust and non-robust measures of center and spread
...
.small[
```{r spread, echo=FALSE,out.width = "74%"}
plot(x, hx, type="l", lty=2, xlab="x value",
...
.pull-left[
- Kurtosis primarily describes the contribution of a distribution's tails to its overall variability.
- Relative to a normal distribution:
    - **Leptokurtic:** heavier tails; positive excess kurtosis
    - **Mesokurtic:** normal-like tails; zero excess kurtosis
    - **Platykurtic:** lighter tails; negative excess kurtosis
- High kurtosis usually reflects rare, extreme observations rather than simply a sharper peak.
]
<br>
.pull-right[
```{r,echo=FALSE}
...
Mode <- function(x) {
  #finds all unique values
	ux <- unique(x)   
	#returns the value which is most frequent
