...
.pull-left[
- In this lab, you will use Excel to explore Z-scores, correlation, and regression with the Iris dataset.
...
We will use the famous **Iris** dataset for this lab. 
...
# Task 1: Working with Z-Scores in Excel


- You’ll work with the Sepal.Length column to calculate Z-scores, percentiles, and inverse normal values.
...
3. Click the cell and observe the Formula Bar at the top of the screen, where you’ll enter the formula.
...
- **Formula Entry**: You can type any valid Excel formula here. In this case, `=STDEV.S()` calculates the sample standard deviation.
...
2. **In the Formula Bar**, type the following formula to calculate the standard deviation for `Sepal.Length`:
   - `=STDEV.S(A2:A151)`
...
**Explanation**:
- **NORM.S.DIST()**: This Excel function calculates the cumulative probability of a Z-score. The `TRUE` argument tells Excel to return the cumulative probability.
]

---

## Step 5: Use `NORM.INV()` to Find Raw Scores for Given Percentiles

- Now, let's reverse the process: Suppose you want to know the raw score (e.g., the `Sepal.Length`) corresponding to a specific percentile, such as the **90th percentile**. 
...
**Explanation**:
- **NORM.INV()** calculates the inverse of the normal cumulative distribution for a given probability. Here, the probability is `0.90`, representing the 90th percentile, and the function will return the raw score (in this case, a `Sepal.Length` value) that corresponds to that percentile.
]
---
...
- Used the **`NORM.INV()`** function to find the raw score corresponding to a given percentile (e.g., the 90th percentile).


---

class: center, middle, inverse
# Task 2: Bivariate Relationships and Correlation
...
We will work with two columns from the **Iris dataset**:
...
- You’ll continue to work with the **Iris dataset**, but this time using two columns: `Sepal.Length` and `Petal.Length` for correlation analysis.
...
[Download the Excel file here](IrisData_Bivariate.xlsx)

- This way easily refer to the data in Excel for the next steps.

---


## Step 2: Create a Scatter Plot in Excel

### Objective: Visualize the Relationship Between Sepal Length and Petal Length
...
4. **Format the Scatter Plot**:
   - Add axis labels:  
     - **X-axis**: Sepal Length  
     - **Y-axis**: Petal Length
   - Give the chart a title, such as “Scatter Plot of Sepal Length vs. Petal Length.”
]

---

## Step 3: Calculate the Correlation Coefficient in Excel
...
image_path <- "../img/excel_lab/excel screenshot_1.png"
img <- image_read(image_path ) %>%
  image_annotate("Calculate Correlation: =CORREL(A2:A151, B2:B151)", size = 30, color = "green",  location = "+550+475")
img
```
]
---
...
[Download the Excel file here](IrisData_Regression.xlsx)

---

## Step 2: Calculate the Slope Using `SLOPE()` in Excel

### Objective: Fit a Linear Regression Model to Predict Petal Length Based on Sepal Length
...
## Step 3: Calculate the Intercept Using `INTERCEPT()`

### Objective: Calculate the Intercept of the Regression Line
...
## Step 4: Calculate the R-squared Value Using `RSQ()`

### Objective: Assess the Fit of the Regression Line
...
## Step 5 (Optional): Use `LINEST()` for Multiple Outputs

### Objective: Calculate Slope, Intercept, and R-Squared Together
...
## Step 6: Use the Regression Equation to Predict Values
...
2. **Example**: 
   - If your equation is \( Y = `r sepal_slope`X + `r sepal_int` \), and you want to predict the `Petal.Length` for a `Sepal.Length` of 7, substitute \( X = 7 \) into the equation:
     \[
...
.reminder[Be sure to save your Excel file with the regression output and predicted values for future reference.]
